...
2. Add AI service domains to `Rules/Custom_AI_Domain.list` using classical Clash rule syntax.
3. Ensure `Yaml/Overwrite-Clash.yaml` has this rule before the generic `category-ai` rule:

```yaml
  - RULE-SET,Custom_AI_Domain,AI服务
...
```yaml
  Custom_AI_Domain:                         {<<: *class,          url: "https://raw.githubusercontent.com/wangqiankun/OpenClash_Overwrite/refs/heads/main/Rules/Custom_AI_Domain.list"}
```
